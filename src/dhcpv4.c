...
{
	if (iface->dhcpv4_event.socket > 0) {
		close(iface->dhcpv4_event.socket);
		iface->dhcpv4_event.socket = -1;
	}
	...
	if (iface->dhcpv4 && enable) {
		...
		if (!iface->dhcpv4_start.s_addr && !iface->dhcpv4_end.s_addr &&
			...
			if (ntohl(mask.s_addr) <= 0xffffffc0) {
				iface->dhcpv4_start.s_addr = addr.s_addr & mask.s_addr;
				iface->dhcpv4_end.s_addr = addr.s_addr & mask.s_addr;

				if (ntohl(mask.s_addr) <= 0xffffff00) {
					iface->dhcpv4_start.s_addr |= htonl(20);
					iface->dhcpv4_end.s_addr |= htonl(199);
				} else {
					iface->dhcpv4_start.s_addr |= htonl(10);
					iface->dhcpv4_end.s_addr |= htonl(59);
				}
			}
		...
		struct relayd_interface *iface)
{
	...
	int sock = iface->dhcpv4_event.socket;
